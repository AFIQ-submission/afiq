...
class LlamaMLP(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.index = LlamaMLP.mlp_index
        LlamaMLP.mlp_index += 1
        self.act_fn = ACT2FN[config.hidden_act]
    ...
    def forward(self, x):
        if self.config.pretraining_tp > 1:
            ...
            if self.profiling and self.first_exec:
                with open(f"{self.log_dir}/dims.csv", "a") as f:
                    f.write(f"{self.index},{len(x.shape)},{len(self.gate_proj.weight.shape)}")
                    for dim in x.shape:
                        f.write(f",{dim}")
                    for dim in self.gate_proj.weight.shape:
                        f.write(f",{dim}")
                    f.write("\n")
                self.first_exec = False

            if self.profiling:
                preact = x @ self.gate_proj.weight.T
                if self.config.mlp_bias:
                    preact += self.gate_proj.bias
                ...
                with open(f"{self.log_dir}/preact_distribution.csv", "a") as f:
                    f.write(f"{self.index},{max_abs},10neg,10pos,")
                    for i in range(10):
                        ii = 9 - i
                        f.write(f"{torch.sum(torch.where((preact < -ii*max_abs/10) & (preact >= -(ii+1)*max_abs/10), 1, 0)).item()},")

                    for i in range(10):
                        f.write(f"{torch.sum(torch.where((preact >= i*max_abs/10) & (preact < (i+1)*max_abs/10), 1, 0)).item()},")
                    f.write("\n")

                # weight_max_abs = torch.max(torch.abs(self.gate_proj.weight), dim=1, keepdim=False).values
            ...
            if self.baseline:
                gate_proj_x = self.gate_proj(x)
            else:
                topk_rows = int(self.gate_proj.weight.shape[-1] * self.topk_percent) + 1
                if len(x.shape) == 2:
                    prod_topk = x[:,:topk_rows] @ self.gate_proj.weight[:,:topk_rows].T
                elif len(x.shape) == 3:
                    prod_topk = x[:,:,:topk_rows] @ self.gate_proj.weight[:,:topk_rows].T
                else:
                    assert False
                prod = x @ self.gate_proj.weight.T
                ...
                if self.config.mlp_bias:
                    prod_topk += self.gate_proj.bias
                    prod += self.gate_proj.bias

                q_x = quantize(x, self.bits, quantization_type="per_channel")
                q_w = quantize(self.gate_proj.weight, self.bits, quantization_type="per_channel")
                if self.config.mlp_bias:
                    q_b = quantize(self.gate_proj.bias, self.bits, quantization_type="per_channel")
                q_x = q_x @ q_w.T
                if self.config.mlp_bias:
                    q_x += q_b

                if self.oracle:
                    gate_proj_x = torch.where(prod<0, q_x, prod)
                    ...
                    elif len(prod.shape) == 2:
                        num_pos_pred = torch.sum(torch.where(prod_topk>0, 1, 0), dim=(0), keepdim=False)
                    fp_channels = num_pos_pred > threshold
                    if len(prod.shape) == 3:
                        num_pos_actual = torch.sum(torch.where(prod>0, 1, 0), dim=(0,1), keepdim=False)
                    elif len(gate_proj_x.shape) == 2:
                        num_pos_actual = torch.sum(torch.where(prod>0, 1, 0), dim=(0), keepdim=False)
                    ...
                    gate_proj_x = prod
                    if len(gate_proj_x.shape) == 3:
                        fp_channels = fp_channels.expand(gate_proj_x.shape[0], gate_proj_x.shape[1], fp_channels.shape[0])
                    elif len(gate_proj_x.shape) == 2:
                        fp_channels = fp_channels.expand(gate_proj_x.shape[0], fp_channels.shape[0])
                    else:
                        assert False

                    gate_proj_x = torch.where(fp_channels, gate_proj_x, q_x)
            down_proj = self.down_proj(self.act_fn(gate_proj_x) * self.up_proj(x))
