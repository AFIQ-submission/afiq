...
class OPTDecoderLayer(nn.Module):
    ...
    def __init__(self, config: OPTConfig):
        ...
        self.index = OPTDecoderLayer.mlp_index
        OPTDecoderLayer.mlp_index += 1
    ...
    def forward(
        ...
        if self.profiling and self.first_exec:
            with open(f"{self.log_dir}/dims.csv", "a") as f:
                f.write(f"{self.index},{len(x.shape)},{len(self.fc1.weight.shape)}")
                for dim in x.shape:
                    f.write(f",{dim}")
                for dim in self.fc1.weight.shape:
                    f.write(f",{dim}")
                f.write("\n")
        ...
        if self.baseline:
            ...
            if self.oracle:
                ...
                x = prod
                fp_channels = fp_channels.expand(x.shape[0], fp_channels.shape[0])
